...
function Programms_report() {
    ...
    const searchFilteredRescueDetails = programms_details.filter((item) => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            const res = await apiRoute.delete(`/remove/CommunityReporttoRecycleBin/${id}`);
                    ...
                    <Col md={12}>
                        <Form>
                            ...
                            {Array.isArray(files.programms_photos) &&
                                files.programms_photos.map((imgUrl, index) => {
                                    const filename = `programms_photos_${index}.jpg`;
