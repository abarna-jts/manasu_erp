...
function Psychiatrics_form() {
    ...
    const formData = useSelector((state) => state.psychiatric);
    const chiefData = useSelector((state) => state.psychiatric);
    const presentingData = useSelector((state) => state.psychiatric);
    const psyHistoryData = useSelector((state) => state.psychiatric);
    const medicalData = useSelector((state) => state.psychiatric);
    const familyData = useSelector((state) => state.psychiatric);
    const socialData = useSelector((state) => state.psychiatric);
    ...
    const handleSuicidalSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response && error.response.data && error.response.data.message) {
                ...
                alert("Error submitting Suicidal and Homicidal Ideation form.");
            }
        }
    }
    ...
    const renderCheckbox = (field, id, label) => {
        const current = Array.isArray(presentingData[field]) ? presentingData[field] : [];
        return (
            <Form.Check
                type="checkbox"
                id={id}
    ...
    const renderpsyCheckbox = (field, id, label) => {
        ...
        return (
            <Form.Check
                ...
                id={id}
                label={label}
                checked={psycurrent.includes(label)}
                onChange={(e) => {
                    const updated = e.target.checked
                        ? [...psycurrent, label]
                        : psycurrent.filter(item => item !== label);
    ...
    const renderMedCheckbox = (field, id, label) => {
        const medcurrent = Array.isArray(medicalData[field]) ? medicalData[field] : [];
        return (
            <Form.Check
                type="checkbox"
                id={id}
                label={label}
                checked={medcurrent.includes(label)}
                onChange={(e) => {
                    const updated = e.target.checked
                        ? [...medcurrent, label]
                        : medcurrent.filter(item => item !== label);
    ...
    const renderFamCheckbox = (field, id, label) => {
        const famcurrent = Array.isArray(familyData[field]) ? familyData[field] : [];
        return (
            <Form.Check
                type="checkbox"
                id={id}
                label={label}
                checked={famcurrent.includes(label)}
                onChange={(e) => {
                    const updated = e.target.checked
                        ? [...famcurrent, label]
                        : famcurrent.filter(item => item !== label);
