import util from 'util';
import multer from 'multer';
import path from 'path';
import fs from 'fs';
// Storage strategy based on fieldname
// Define multer storage
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        let uploadPath;

        // Map field names to folders
        if (['f_aadhar_card', 'f_ration_card', 'govt_id', 'r_aadhar_card', 'r_ration_card'].includes(file.fieldname)) {
            uploadPath = path.resolve('uploads/Reunion/Family_Details/');
        } else if (['signature', 'photo', 'handwritten_document'].includes(file.fieldname)) {
            uploadPath = path.resolve('uploads/Self_Declaration/');
        } else if (['scan_report'].includes(file.fieldname)) {
            uploadPath = path.resolve('uploads/MediaConsent/');
        } else {
            // Default fallback for Discharge_Checklist and others
            uploadPath = path.resolve('uploads/Reunion/Discharge_Checklist/');
        }

        // Create the folder if it doesn't exist
        fs.mkdirSync(uploadPath, { recursive: true });

        // Pass the folder to multer
        cb(null, uploadPath);
    },
    filename: function (req, file, cb) {
        cb(null, Date.now() + '-' + file.originalname);
    }
});

// Define upload fields
const upload = multer({ storage: storage }).fields([
    { name: 'f_aadhar_card', maxCount: 15 },
    { name: 'f_ration_card', maxCount: 15 },
    { name: 'r_aadhar_card', maxCount: 15 },
    { name: 'r_ration_card', maxCount: 15 },
    { name: 'govt_id', maxCount: 15 },
    { name: 'signature', maxCount: 15 },
    { name: 'photo', maxCount: 15 },
    { name: 'handwritten_document', maxCount: 15 },
    { name: 'scan_report', maxCount: 15 },
    { name: 'familyRequestLetterFile', maxCount: 15 },
    { name: 'selfDeclarationFile', maxCount: 15 },
    { name: 'mediaConsentFile', maxCount: 15 },
    { name: 'familyIDproofFile', maxCount: 15 },
    { name: 'aadharCardFile', maxCount: 15 },
    { name: 'udidCardFile', maxCount: 15 },
    { name: 'disabilityCertificateFile', maxCount: 15 },
    { name: 'bankPassbookFile', maxCount: 15 },
    { name: 'healthInsuranceFile', maxCount: 15 },
    { name: 'medicalReportFile', maxCount: 15 },
    { name: 'dischargeSummaryFile', maxCount: 15 },
    { name: 'medicationsFile', maxCount: 15 },
    { name: 'ClothesFile', maxCount: 15 },
    { name: 'possessionsRecoveredFile', maxCount: 15 },
    { name: 'dischargeAllowanceFile', maxCount: 15},
    { name: 'travelExpensesFile', maxCount: 15 },
    { name: 'copyOfdischargeSummaryFile', maxCount: 15 },
    { name: 'travelSafetyLetterFile', maxCount: 15 },
    { name: 'reunionPhotoFile', maxCount: 15 },
    { name: 'witnessSignatureFile', maxCount: 15 },
    { name: 'residentIDproofFile', maxCount: 15 },
]);

// ✅ Now promisify AFTER upload is declared
const ReunionAsync = util.promisify(upload);

export{ upload, ReunionAsync };
